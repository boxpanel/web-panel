...
is_rockchip_device() {
    ...
    if [ -f /proc/device-tree/compatible ] && grep -qi rockchip /proc/device-tree/compatible 2>/dev/null; then
        return 0
    fi
    if [ -f /proc/cpuinfo ] && grep -qi rockchip /proc/cpuinfo 2>/dev/null; then
        return 0
    fi
    return 1
}
...
has_rkmpp_support() {
    ...
    ffmpeg -hide_banner -decoders 2>/dev/null | grep -q "hevc_rkmpp" && \
    ffmpeg -hide_banner -encoders 2>/dev/null | grep -q "h264_rkmpp"
}

install_ffmpeg_standard() {
    print_message "安装系统ffmpeg/ffprobe..."
    case "$SYSTEM" in
        "debian")
            $PM install -y ffmpeg >/dev/null 2>&1 || true
            ;;
        "rhel")
            $PM install -y ffmpeg >/dev/null 2>&1 || true
            ;;
        "suse")
            ...
            ;;
        "alpine")
            $PM add ffmpeg >/dev/null 2>&1 || true
            ;;
        "arch")
            $PM -S --noconfirm ffmpeg >/dev/null 2>&1 || true
            ;;
        *)
            print_warning "未知系统，跳过ffmpeg安装"
            ;;
    esac
}
...
install_ffmpeg_rockchip_from_source() {
    ...
    if [ "$SYSTEM" != "debian" ]; then
        print_warning "当前系统非Debian系，暂不自动编译ffmpeg-rockchip，回退安装系统ffmpeg"
        return 1
    fi

    ARCH=$(uname -m 2>/dev/null || echo unknown)
    if [ "$ARCH" != "aarch64" ] && [ "$ARCH" != "arm64" ]; then
        print_warning "当前架构($ARCH)非arm64，跳过ffmpeg-rockchip源码编译安装"
        return 1
    fi

    print_message "检测到Rockchip设备，尝试编译安装ffmpeg-rockchip（用于H265硬件转码）..."

    $PM install -y build-essential pkg-config git yasm nasm cmake >/dev/null 2>&1 || true
    ...
    if ! git clone --depth 1 https://github.com/nyanmisaka/ffmpeg-rockchip.git ffmpeg-rockchip >/dev/null 2>&1; then
        print_warning "克隆ffmpeg-rockchip失败，回退安装系统ffmpeg"
        return 1
    fi

    cd "$BUILD_DIR/ffmpeg-rockchip" || return 1

    if [ ! -x "./configure" ]; then
        print_warning "ffmpeg-rockchip源码目录未找到configure脚本，回退安装系统ffmpeg"
        return 1
    fi
    ...
    print_warning "ffmpeg-rockchip安装完成但未检测到rkmpp编解码器，回退安装系统ffmpeg"
    return 1
}
